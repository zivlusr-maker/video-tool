// VVT API Proxy v4 - Claude + Gemini + Veo
export default async function handler(req, res) {
  ...
  // 健康检查 - 返回版本号方便确认部署
  if (req.body && req.body.__ping) return res.status(200).json({ ok: true, version: 'v4' });

  const gKey = process.env.GOOGLE_API_KEY || process.env.GEMINI_API_KEY;

  // Veo 视频生成 (predictLongRunning, instances+parameters格式)
  if (req.body && req.body.__veo) {
    ...
    try {
      const r = await fetch(
        ...
        { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ instances, parameters }) }
      );
